...
- 多特性一起做时，**每个特性各自产出一份独立的特性需求文档，绝不把多个需求混写进同一份**——逐个走完后续步骤，彼此只用「关联特性」互相指向、标清依赖（如 D 依赖 A+B）。（注意：跨层特性的"主档案 + 各层子需求档"是**同一个特性内部按实现层拆**，不是把多个特性合并——两者不冲突。）
...
**产出形态（跨层特性）：** 跨层（结论 B/C）特性按**「主档案 + 各层子需求档」**产出——主档案是面向跨部门协同的总览（含《跨层实现与跨部门协同》节）；每个**需改动的层**另出一份可单独交付给该部门的**子需求档**（模板见 `references/cross-layer-routing.md`《九》），主档案在分解表里链接各子档。单层（结论 A）特性退化为单份文档，不产子档。
...
- **一份文档只承载一个特性**；需求分流出的多个特性各自独立成文、绝不混写进同一份（跨层特性的「主档案 + 各层子需求档」是同一特性内按层拆，不是多特性合并）。
- 跨层（结论 B/C）特性产出**「主档案 + 各层子需求档」**：本模板是主档案；各需改动层另出子需求档（模板见 references/cross-layer-routing.md《九》）。
...
> **产出形态：** 跨层特性产出「主档案 + 各层子需求档」——本节是主档案的协同视图；每个需改动的层另出一份可单独交付给该部门的《子需求档》（模板见 references/cross-layer-routing.md《九》），下面分解表的「owner」列链接到对应子档。
