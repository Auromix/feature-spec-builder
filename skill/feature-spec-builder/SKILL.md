...
description: 把客户或合作伙伴抛来的一句话功能需求（例如"我需要一个迎宾导览功能""能不能支持某某能力"）细化成研发可以直接接手的特性需求文档。当用户粘贴一段零散的、口语化的、还没想清楚的功能诉求，并希望把它变成带能力定位、范围边界、接口契约草案和可验证验收标准的规格时，使用本技能。只要用户提到"把这个需求细化""写成研发能看的需求""这个 feature 怎么定义""客户想要 XX 功能"之类，即使没明说要"特性需求文档"，也应使用本技能。本技能面向 SDK / 平台型产品的二次开发特性需求场景；并能在需求跨多实现层（二开 SDK / 底层软件(固件·中间件) / 算法服务 / 硬件）、需要识别各层 owner、跨层依赖顺序与层间接口契约，或需判断某需求"是不是光靠二开就能独立交付"（例如客户要某个接口，但其实强依赖固件/算法/硬件是否已支持）时，给出跨层实现分解与跨部门协同视图。此外，当用户提供或粘贴**本公司自己的组织/流程/版本管理资料**（人员职责分工、人员地图、发版节奏、责任矩阵、分支合入规范等）、说"这是我们公司的资料/情况"并希望需求规划贴合公司协作时，也应使用本技能——把这些资料落成一份**本地私有的组织流程配置**（脱敏、不入库、不提交），供后续需求细化按公司角色地图与流程 gate 做 owner 路由与版本协同。
---
...
1. **逐层判定要不要动**：对二开 SDK 层 / 底层软件(固件·中间件) / 算法服务层 / 硬件层四个**运行时层**，逐层问"这一层需要改什么"。**另外必问两类非运行时层**（生态/集成/文档类特性重心常在这里）：**文档门户层**（官网板块、示例仓、文档 IA——owner 是 DevRel/文档/站点，不是研发）与**构建/分发基建层**（打包、多环境 CI、ABI/预编译——owner 是 SDK 构建/DevOps）。⚠ 交付物是**示例/打包/安装/分发/文档**时（原型 H/I），即使技术触底=纯二开，也**不要默认派给二开 SDK 组**——按配置区 E 组路由到对应 owner（硬塞给固件/算法/硬件组、或默认丢给二开组，都是高危错派）。能干净答"只动二开层、无门户/基建改动"的，记"单层特性（二开 SDK）"，直接进第 5 步。
