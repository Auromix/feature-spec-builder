...
1. **逐层判定要不要动**：对二开 SDK / 底层软件(固件·中间件) / 算法服务 / 硬件四层，逐层问"这一层需要改什么"。能干净答"只动二开层"的，记一句"单层特性（二开 SDK）"，直接进第 5 步。
...
> **三条正交的轴，叠加使用：** 上面九维 + detail-probes 的「特性原型 A~G」是按**特性类型**切的一条轴；某特性还要不要**跨实现层分解**（动了二开层以外的层）是第二条轴，见第 4 步与 `references/cross-layer-routing.md`；有哪些**非功能性需求(NFR) 与产品原则**要过是第三条轴，见 `references/product-principles.md`。三者不互相替代，命中即叠加。
